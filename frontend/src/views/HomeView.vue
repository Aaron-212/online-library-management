      ...
      <div class="relative z-10">
        ...
        <div class="mt-8 max-w-xl">
          <div class="flex items-center gap-2 p-2 border rounded-md bg-background">
            ...
            <Input v-model="searchKeyword" :placeholder="t('home.hero.searchPlaceholder')"
              class="flex-1 border-0 focus-visible:ring-0 focus-visible:ring-offset-0" @keyup.enter="handleSearch" />
    ...
    <template v-else>
      ...
      <section v-if="recentNotices.length" class="hidden sm:block">
        <div class="overflow-x-auto flex gap-6 snap-x pb-4 scrollbar-hide">
          <Card v-for="notice in recentNotices" :key="notice.id"
      ...
      <section v-if="recommendedBooks.length > 0">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          <Card v-for="book in recommendedBooks" :key="book.id"
            ...
            <CardHeader class="pb-3">
              <div class="flex items-start justify-between">
                ...
                <Badge :variant="book.availableQuantity > 0 ? 'success' : 'destructive'" class="ml-2 shrink-0">
                  {{ book.availableQuantity > 0 ? t('home.status.available') : t('home.status.outOfStock') }}
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
              <div class="flex items-center justify-between text-xs text-muted-foreground">
                <span>{{ t('home.status.general') }}</span>
      ...
      <section v-if="authStore.isAuthenticated">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Button v-for="action in quickActions" :key="action.title" :variant="action.variant"
            class="h-auto p-6 flex flex-col items-center gap-3 text-center" @click="action.action">
